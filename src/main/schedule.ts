...
export function getScheduleStatus(workflow: Workflow): ScheduleStatus {
  ...
  if (res.error)
    return {
      installed: false,
      loaded: false,
      error: String(res.error.message),
    };
  if (res.status !== 0) {
    return {
      installed: false,
      loaded: false,
      error: res.stderr?.trim() || "Status check failed.",
    };
  }
...
export function enableSchedule(workflow: Workflow): ScheduleStatus {
  ...
  if (res.status !== 0) {
    return {
      installed: false,
      loaded: false,
      error: res.stderr?.trim() || "Failed to enable schedule.",
    };
  }
...
export function disableSchedule(workflow: Workflow): ScheduleStatus {
  ...
  if (res.status !== 0) {
    return {
      installed: false,
      loaded: false,
      error: res.stderr?.trim() || "Failed to disable schedule.",
    };
  }
  return getScheduleStatus(workflow);
}
