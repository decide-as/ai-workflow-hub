...
export function Sidebar({ clusters, selected, onSelect, totalCount, selectedType, onSelectType, typeCounts }: Props) {
  return (
        ...
        <div className="space-y-0.5">
          {TYPE_OPTIONS.filter((t) => typeCounts[t.id] > 0).map((t) => {
            ...
            return (
              ...
              >
                <t.Icon size={13} className="shrink-0" style={{ color: active ? t.color : undefined }} />
                <span className="flex-1 text-left font-medium truncate">{t.label}</span>
                <span className="text-[11px] text-zinc-600 tabular-nums">{typeCounts[t.id]}</span>
              </button>
            )
          })}
        </div>
