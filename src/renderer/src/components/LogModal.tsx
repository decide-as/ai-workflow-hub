...
function parseRuns(content: string): RunEntry[] {
  ...
  function parseBlock(blockLines: string[]): Omit<RunEntry, 'index'> {
    ...
    for (const line of blockLines) {
      const startMatch = line.match(/RUN STARTED:\s*(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})/)
      if (startMatch) {
  ...
  if (startIndices.length === 0) {
    ...
    startIndices.forEach((startIdx, i) => {
      const endIdx = i + 1 < startIndices.length ? startIndices[i + 1] : lines.length
...
export function LogModal({ workflowName, logPath, color, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
      ...
      >
        <div className="h-1 w-full rounded-t-2xl shrink-0" style={{ backgroundColor: color }} />
        ...
        <div className="flex items-center gap-3 px-6 pt-5 pb-4 shrink-0">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-sm font-semibold text-zinc-100">{workflowName} — Run Log</h2>
            {logPath && (
              <p className="text-[11px] text-zinc-600 font-mono truncate mt-0.5">{logPath}</p>
            )}
          </div>
          <button
            onClick={onClose}
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b border-zinc-800">
                  <th className="text-left text-[11px] font-semibold uppercase tracking-wider text-zinc-600 py-2 pr-4 w-8">#</th>
                  <th className="text-left text-[11px] font-semibold uppercase tracking-wider text-zinc-600 py-2 pr-4">Time</th>
                  <th className="text-left text-[11px] font-semibold uppercase tracking-wider text-zinc-600 py-2 pr-4">Result</th>
                  <th className="text-right text-[11px] font-semibold uppercase tracking-wider text-zinc-600 py-2">Files moved</th>
                </tr>
              </thead>
              <tbody>
                {runs.map((run) => (
                  <tr key={run.index} className="border-b border-zinc-800/50 hover:bg-zinc-800/20 transition-colors">
                    <td className="py-2.5 pr-4 text-zinc-600 tabular-nums text-xs">{run.index}</td>
                    <td className="py-2.5 pr-4 text-zinc-400 tabular-nums text-xs">
                      {run.timestamp ?? <span className="text-zinc-600 italic">unknown</span>}
                    </td>
                    ...
                    <td className="py-2.5 text-right text-zinc-300 tabular-nums text-xs">
                      {run.moved}
                      {run.removed > 0 && (
                        <span className="text-zinc-600 ml-1.5">+{run.removed} folders cleaned</span>
                      )}
                    </td>
                  </tr>
                ))}
