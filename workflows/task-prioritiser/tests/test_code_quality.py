"""Automated code quality checks for Task-Prioritiser.
...
SRC_ROOT = Path(__file__).resolve().parent.parent / "src" / "task_prioritiser"
...
class TestCoverage:
    ...
    def test_every_module_has_test_file(self) -> None:
        ...
        assert not missing, (
            ...
            + "\n".join(f"  src/task_prioritiser/{m}.py -> tests/test_{m}.py" for m in missing)
        )
