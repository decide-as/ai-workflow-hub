"""Automated code quality checks for Meeting-Notes-Summariser.
...
SRC_ROOT = Path(__file__).resolve().parent.parent / "src" / "meeting_notes_summariser"
...
class TestCoverage:
    ...
    def test_every_module_has_test_file(self) -> None:
        ...
        assert not missing, (
            ...
            + "\n".join(f"  src/meeting_notes_summariser/{m}.py -> tests/test_{m}.py" for m in missing)
        )
