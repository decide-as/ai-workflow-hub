"""Automated code quality checks for Gdpr-Compliance-Checker.
...
SRC_ROOT = Path(__file__).resolve().parent.parent / "src" / "gdpr_compliance_checker"
...
class TestCoverage:
    ...
    def test_every_module_has_test_file(self) -> None:
        ...
        assert not missing, (
            ...
            + "\n".join(f"  src/gdpr_compliance_checker/{m}.py -> tests/test_{m}.py" for m in missing)
        )
