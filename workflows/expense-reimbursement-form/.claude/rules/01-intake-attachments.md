...
Process every file the user provides for the batch (from `inbox/`, pasted images,
or given paths).
...
```json
{
  "attachments": {
    "9f1c…": {
      "original_name": "kvittering_rema.pdf",
      "ext": "pdf",
      "sha256": "…",
      "converted_from": null,
      "number": null
...
- an **order confirmation** plus the **itemised receipt** for the same purchase;
- two screenshots of the same charge from different apps.
...
not intake order. They become the **Bilagsnr** (receipt no.) in the form.
...
4. Set each transaction's `attachment_no` (in `report.json`) to its new numbers,
   comma-separated (e.g. `01, 02`). This is what goes in the **Receipt no. /
   Bilagsnr** column.
