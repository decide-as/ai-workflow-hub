"""Automated code quality checks for Proposal-Writer.
...
SRC_ROOT = Path(__file__).resolve().parent.parent / "src" / "proposal_writer"
...
class TestCoverage:
    ...
    def test_every_module_has_test_file(self) -> None:
        ...
        assert not missing, (
            ...
            + "\n".join(f"  src/proposal_writer/{m}.py -> tests/test_{m}.py" for m in missing)
        )
