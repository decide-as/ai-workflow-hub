"""Automated code quality checks for Project-Status-Reporter.
...
SRC_ROOT = Path(__file__).resolve().parent.parent / "src" / "project_status_reporter"
...
class TestCoverage:
    ...
    def test_every_module_has_test_file(self) -> None:
        ...
        assert not missing, (
            ...
            + "\n".join(f"  src/project_status_reporter/{m}.py -> tests/test_{m}.py" for m in missing)
        )
