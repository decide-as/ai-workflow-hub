"""Automated code quality checks for Contract-Reviewer.
...
SRC_ROOT = Path(__file__).resolve().parent.parent / "src" / "contract_reviewer"
...
class TestCoverage:
    ...
    def test_every_module_has_test_file(self) -> None:
        ...
        assert not missing, (
            ...
            + "\n".join(f"  src/contract_reviewer/{m}.py -> tests/test_{m}.py" for m in missing)
        )
