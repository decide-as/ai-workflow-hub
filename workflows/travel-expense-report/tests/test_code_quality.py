"""Automated code quality checks for Travel-Expense-Report.
...
SRC_ROOT = Path(__file__).resolve().parent.parent / "src" / "travel_expense_report"
...
class TestCoverage:
    ...
    def test_every_module_has_test_file(self) -> None:
        ...
        assert not missing, (
            ...
            + "\n".join(f"  src/travel_expense_report/{m}.py -> tests/test_{m}.py" for m in missing)
        )
