"""Automated code quality checks for Client-Email-Drafter.
...
SRC_ROOT = Path(__file__).resolve().parent.parent / "src" / "client_email_drafter"
...
class TestCoverage:
    ...
    def test_every_module_has_test_file(self) -> None:
        ...
        assert not missing, (
            ...
            + "\n".join(f"  src/client_email_drafter/{m}.py -> tests/test_{m}.py" for m in missing)
        )
