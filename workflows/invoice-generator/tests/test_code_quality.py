"""Automated code quality checks for Invoice-Generator.
...
SRC_ROOT = Path(__file__).resolve().parent.parent / "src" / "invoice_generator"
...
class TestCoverage:
    ...
    def test_every_module_has_test_file(self) -> None:
        ...
        assert not missing, (
            ...
            + "\n".join(f"  src/invoice_generator/{m}.py -> tests/test_{m}.py" for m in missing)
        )
