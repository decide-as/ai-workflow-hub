"""Automated code quality checks for Team-Briefing-Generator.
...
SRC_ROOT = Path(__file__).resolve().parent.parent / "src" / "team_briefing_generator"
...
class TestCoverage:
    ...
    def test_every_module_has_test_file(self) -> None:
        ...
        assert not missing, (
            ...
            + "\n".join(f"  src/team_briefing_generator/{m}.py -> tests/test_{m}.py" for m in missing)
        )
