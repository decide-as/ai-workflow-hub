...
This document classifies every test in the Budget-Variance-Analyst test suite by category, provides counts, and identifies gaps. Tests are also marked with `@pytest.mark.<category>` decorators that enable running subsets via `pytest -m <marker>`.
