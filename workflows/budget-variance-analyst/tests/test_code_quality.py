"""Automated code quality checks for Budget-Variance-Analyst.
...
SRC_ROOT = Path(__file__).resolve().parent.parent / "src" / "budget_variance_analyst"
...
class TestCoverage:
    ...
    def test_every_module_has_test_file(self) -> None:
        ...
        assert not missing, (
            ...
            + "\n".join(f"  src/budget_variance_analyst/{m}.py -> tests/test_{m}.py" for m in missing)
        )
