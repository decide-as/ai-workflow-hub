...
program
  ...
  .option('--icon <name>', 'Lucide icon name (e.g. Plane, Mail, Receipt)', 'Bot')
  .action((opts) => {
    ...
    const clusterName = saved.clusters.find((c) => c.workflow_ids.includes(workflow.id))?.name
...
program
  ...
  .action((id, opts) => {
    ...
    if (opts.tags) w.tags = opts.tags.split(',').map((t: string) => t.trim()).filter(Boolean)
...
program
  ...
  .action((opts) => {
    ...
    for (const w of workflows) {
      ...
      console.log(`   tags: ${w.tags.join(', ')}  workspace: ${c?.name ?? '—'}`)
